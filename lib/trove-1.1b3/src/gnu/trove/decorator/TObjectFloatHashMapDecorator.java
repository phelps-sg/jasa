 ...
 * @since trove 0.1.8
 */
public class TObjectFloatHashMapDecorator extends AbstractMap implements Map, Cloneable {
    /** the wrapped primitive map */
    protected TObjectFloatHashMap _map;

    /**
     * Creates a wrapper that decorates the specified primitive map.
     */
    public TObjectFloatHashMapDecorator(TObjectFloatHashMap map) {
	super();
	this._map = map;
    }
	...
	if (_map.equals(other)) {
		...
		for (int i = that.size(); i-- > 0;) {
		    ...
		    if (key instanceof Object && val instanceof Float) {
    ...
    public Set entrySet() {
		...
		public Iterator iterator() {
		    return new Iterator() {
			    private final TObjectFloatIterator it = _map.iterator();
