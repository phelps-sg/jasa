 ...
 * @since trove 0.1.8
 */
public class TIntObjectHashMapDecorator extends AbstractMap implements Map, Cloneable {
    /** the wrapped primitive map */
    protected TIntObjectHashMap _map;

    /**
     * Creates a wrapper that decorates the specified primitive map.
     */
    public TIntObjectHashMapDecorator(TIntObjectHashMap map) {
	super();
	this._map = map;
    }
	...
	if (_map.equals(other)) {
		...
		for (int i = that.size(); i-- > 0;) {
		    ...
		    if (key instanceof Integer && val instanceof Object) {
			int k = unwrapKey(key);
    ...
    public Set entrySet() {
		...
		public Iterator iterator() {
		    return new Iterator() {
			    private final TIntObjectIterator it = _map.iterator();
