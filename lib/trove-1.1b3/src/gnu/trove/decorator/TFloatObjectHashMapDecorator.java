 ...
 * @since trove 0.1.8
 */
public class TFloatObjectHashMapDecorator extends AbstractMap implements Map, Cloneable {
    /** the wrapped primitive map */
    protected TFloatObjectHashMap _map;

    /**
     * Creates a wrapper that decorates the specified primitive map.
     */
    public TFloatObjectHashMapDecorator(TFloatObjectHashMap map) {
	super();
	this._map = map;
    }
	...
	if (_map.equals(other)) {
		...
		for (int i = that.size(); i-- > 0;) {
		    ...
		    if (key instanceof Float && val instanceof Object) {
			float k = unwrapKey(key);
			Object v = unwrapValue(val);
    ...
    public Set entrySet() {
		...
		public Iterator iterator() {
		    return new Iterator() {
			    private final TFloatObjectIterator it = _map.iterator();
