 ...
 * <p>An EvolutionState object contains quite a few objects, including:
 ...
 <li> Whether to garbage collect each generation.
 <li> Whether that garbage collection should be aggressive.
 ...
 <tr><td valign=top><tt>gc</tt><br>
 <font size=-1>bool = <tt>true</tt> or <tt>false</tt> (default)</td>
 <td valign=top>(should we force GC?)</td></tr>
 
 <tr><td valign=top><tt>gc-modulo</tt><br>
 <font size=-1>int &gt;= 1</td>
 <td valign=top>(how many generations should pass before we do garbage collection?
 The definition of "generations" depends on the particular EvolutionState 
 implementation you're using)</td></tr>
 
 <tr><td valign=top><tt>aggressive</tt><br>
 <font size=-1>bool = <tt>true</tt> or <tt>false</tt> (default)</td>
 <td valign=top>(if we force GC (see <tt>gc</tt>), should it be aggressive?)</td></tr>
 ...
 <font size=-1>classname, inherits and != ec.Statistics</font></td>
...
public abstract class EvolutionState implements Singleton
    {
    ...
    /** Whether or not the system should be doing <i>aggressive</i> forced garbage collection after each generation.  "Aggressive" means that garbage collection is repeatedly performed until there is absolutely no memory left to reclaim.  This flag has no effect unless <tt>gc</tt> is also set to true.  You probably shouldn't modify this during a run.*/
    ...
    public final static String P_GC = "gc";
    public final static String P_AGGRESSIVEGC = "aggressive";
    ...
    public final static String P_GCMODULO = "gc-modulo";
    ...
    */
    public void setup(final EvolutionState state, final Parameter base)
        ...
        p = new Parameter(P_GCMODULO);
        gcModulo = parameters.getInt(p,null,1);
        if (gcModulo==0)
            output.fatal("The GC modulo must be an integer >0.",p);
        ...
        p = new Parameter(P_GC);
        gc = parameters.getBoolean(p,null,false);
        
        p=new Parameter(P_AGGRESSIVEGC);
        aggressivegc = parameters.getBoolean(p,null,false);
        ...
        statistics = (Statistics)
            (parameters.getInstanceForParameter(p,null,Statistics.class));
    ...
    public abstract void run(final int condition) throws IOException;


    /** Steps through a run <i>n</i> times for debugging purposes.
        A debug-run is done by repeatedly pulsing the go() method.  The
        default form prints a message saying that nothing happens. */

    public void go(int n)
        { for(int x=0;x<n;x++) go(); }
    ...
    public void go()
        { output.message("Not Implemented for this kind of Evolution"); }


    /** Performs nonagressive garbage collection.  A convenience function. */
    public final void gc()
        {
        output.message("Garbage Collecting...");
        Runtime rt = Runtime.getRuntime();
        output.message("Before Collection: " + rt.freeMemory() + "/" + rt.totalMemory());
        rt.runFinalization();
        rt.gc();
        output.message("After Collection:  " + rt.freeMemory() + "/" + rt.totalMemory());
        }
    ...
    public final void aggressiveGC()
        {
        int x=0;
        output.message("Garbage Collecting (Aggressively)...");
        ...
        output.message("After Collection:  " + rt.freeMemory()  + "/" + rt.totalMemory() + " (" + x + " collections)");
        }
