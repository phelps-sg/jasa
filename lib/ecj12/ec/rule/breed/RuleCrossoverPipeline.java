...
public class RuleCrossoverPipeline extends BreedingPipeline
    ...
    public Object protoClone() throws CloneNotSupportedException
        {
        RuleCrossoverPipeline c = (RuleCrossoverPipeline)(super.protoClone());
        ...
        for(int q=start;q<n+start; /* no increment */)  // keep on going until we're filled up
            ...
            if (sources[0]==sources[1])  // grab from the same source
                ...
                if (!(sources[0] instanceof BreedingPipeline))  // it's a selection method probably
                    { 
                    parents[0] = (RuleIndividual)(parents[0].deepClone());
                    parents[1] = (RuleIndividual)(parents[1].deepClone());
                    }
                }
            ...
            for( int x = 0 ; x < parents[0].rulesets.length ; x++ )
                ...
                while(true)
                    ...
                    if (temp[0].numRules >= parents[0].rulesets[x].constraints().minSize &&
                        temp[0].numRules <= parents[0].rulesets[x].constraints().maxSize &&
                        temp[1].numRules >= parents[1].rulesets[x].constraints().minSize &&
                        temp[1].numRules <= parents[1].rulesets[x].constraints().maxSize)
                        break;
