...
/**
   ...
   If NUMTREESOFTYPE[type,size] not defined,
...
// check to make sure we can put 0% probability items
// in our probability distributions

Func NumTreesRootedByNode(node,size)
If NUMTREESROOTEDBYNODE[node,size] not defined,
count = 0
left = size - 1
...
If NUMCHILDPERMUTATIONS[parent,size,outof,pickchild] is not defined,
count = 0
...
public class Uniform extends GPNodeBuilder 
    ...
    public void preprocess(final EvolutionState state, final int _maxtreesize)
        ...
        functionsets = new GPFunctionSet[GPFunctionSet.all.size()];
        _functionsets = new Hashtable();
        Enumeration e = GPFunctionSet.all.elements();
        int count=0;
        ...
        // set up the arrays
        NUMTREESOFTYPE = new BigInteger[functionsets.length][GPType.numAtomicTypes+GPType.numSetTypes][maxtreesize+1];
        ...
        ROOT_D = new UniformGPNodeStorage[functionsets.length][GPType.numAtomicTypes+GPType.numSetTypes][maxtreesize+1][];
        ROOT_D_ZERO = new boolean[functionsets.length][GPType.numAtomicTypes+GPType.numSetTypes][maxtreesize+1];
        ...
        _truesizes = new BigInteger[functionsets.length][GPType.numAtomicTypes+GPType.numSetTypes][maxtreesize+1];
        for(int x=0;x<functionsets.length;x++)
            for(int y=0;y<GPType.numAtomicTypes+GPType.numSetTypes;y++)
                for(int z=1;z<=maxtreesize;z++)
                    state.output.message("Set: " + x + " Type: " + y + " Size: " + z + " num: " + 
                                         (_truesizes[x][y][z] = numTreesOfType(x,y,z)));

        state.output.message("Compiling Distributions");

        // convert to doubles and organize distribution
        truesizes = new double[functionsets.length][GPType.numAtomicTypes+GPType.numSetTypes][maxtreesize+1];
        for(int x=0;x<functionsets.length;x++)
            for(int y=0;y<GPType.numAtomicTypes+GPType.numSetTypes;y++)
                {
                for(int z=1;z<=maxtreesize;z++)
    ...
    public BigInteger numTreesOfType(final int functionset, final int type, final int size)
        {
        if (NUMTREESOFTYPE[functionset][type][size]==null)
            ...
            for(int x=0;x<nodes.length;x++)
                count = count.add(numTreesRootedByNode(functionset,nodes[x].node,size));
            NUMTREESOFTYPE[functionset][type][size] = count;
            }
        return NUMTREESOFTYPE[functionset][type][size];
        }
    
    public BigInteger numTreesRootedByNode(final int functionset, final GPNode node, final int size)
        {
        if (NUMTREESROOTEDBYNODE[functionset][intForNode(node)][size]==null)
            ...
            if (node.children.length == 0 && outof == 0) // a valid terminal
                ...
                for (int s=1;s<=outof;s++)
                    count = count.add(numChildPermutations(functionset,node,s,outof,0));
    ...
    public BigInteger numChildPermutations(
        ...
        if (NUMCHILDPERMUTATIONS[functionset][intForNode(parent)][size][outof][pickchild]==null)
            ...
            if (pickchild == parent.children.length - 1 && size==outof)
                count = numTreesOfType(functionset,parent.constraints().childtypes[pickchild].type,size);
            else if (pickchild < parent.children.length - 1 && 
                     outof-size >= (parent.children.length - pickchild-1))
                {
                BigInteger cval = numTreesOfType(functionset,parent.constraints().childtypes[pickchild].type,size);
                BigInteger tot = BigInteger.valueOf(0);
                for (int s=1; s<=outof-size; s++)
                    tot = tot.add(numChildPermutations(functionset,parent,s,outof-size,pickchild+1));
    ...
    GPNode createTreeOfType(final int functionset, final int type, final int size, final MersenneTwisterFast mt)
        ...
        GPNode node = (GPNode)(ROOT_D[functionset][type][size][choice].node.protoClone());
        ...
        if (size > 1)  // nonterminal
            fillNodeWithChildren(functionset,node,ROOT_D[functionset][type][size][choice].node,0,size-1,mt);
        return node;
        }
       
    void fillNodeWithChildren(final int functionset, final GPNode parent, final GPNode parentc, 
        ...
        if (pickchild == parent.children.length - 1)
            ...
            parent.children[pickchild] = 
                createTreeOfType(functionset,parent.constraints().childtypes[pickchild].type,outof, mt);
            }
        else 
            {
            int size = RandomChoice.pickFromDistribution(
                ...
                mt.nextDouble(),CHECKBOUNDARY);
            parent.children[pickchild] = 
                createTreeOfType(functionset,parent.constraints().childtypes[pickchild].type,size,mt);
            fillNodeWithChildren(functionset,parent,parentc,pickchild+1,outof-size,mt);
            }
    ...
    public GPNode newRootedTree(final EvolutionState state,
        ...
        if (requestedSize == NOSIZEGIVEN)  // pick from the distribution
            ...
            while(ROOT_D_ZERO[fset][typ][siz])
                ...
                if (++bound == BOUNDARY)
                    ...
                    for(int x=0;x<ROOT_D_ZERO[fset][typ].length;x++)
                        if (!ROOT_D_ZERO[fset][typ][x]) break;
            ...
            GPNode n = createTreeOfType(fset,typ,siz,state.random[thread]);
            n.parent = parent;
            ...
            // we need to modify the requestedSize.
