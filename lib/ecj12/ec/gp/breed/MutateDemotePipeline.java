...
public class MutateDemotePipeline extends GPBreedingPipeline
    ...
    public Object protoClone() throws CloneNotSupportedException
        { 
        MutateDemotePipeline obj = (MutateDemotePipeline)(super.protoClone());
        obj.gatherer = new GPNodeGatherer();  // make sure they're different
        return obj;
        }


    private boolean demotable(final GPNode node, final GPFunctionSet set)
        {
        GPType t;

        if (node.parent instanceof GPNode)  // ugh, expensive
            t = ((GPNode)(node.parent)).constraints().childtypes[node.argposition];
        else 
            t = ((GPTree)(node.parent)).constraints().treetype;
        ...
        for(int x=0;x<set.nonterminals[t.type].length;x++)
            for(int y=0;y<set.nonterminals[t.type][x].node.constraints().
                    childtypes.length;y++)
                if (set.nonterminals[t.type][x].node.constraints().childtypes[y].
                    compatibleWith(node.constraints().returntype))
                    return true;
        return false;
        }
    

    private void demoteSomething(final GPNode node, final EvolutionState state, final int thread, final GPFunctionSet set) throws CloneNotSupportedException
        {
        // if I have just one type, do it the easy way
        if (GPType.numAtomicTypes + GPType.numSetTypes == 1)
    ...
    private void _demoteSomething(final GPNode node, final EvolutionState state, final int thread, final GPFunctionSet set) throws CloneNotSupportedException
        {
        int numDemotable = 0;
            ...
            for(int y=0;y<set.nonterminals[t.type][x].node.constraints().
                ...
                if (set.nonterminals[t.type][x].node.constraints().childtypes[y].
                    ...
                    if (numDemotable==demoteItem)
                        ...
                        int numSpots=0;
                        GPType retyp = node.constraints().returntype;
                        GPType[] chityp = cnode.constraints().childtypes;

                        for(int z=0;z<cnode.children.length;z++)
                            if (chityp[z].compatibleWith(retyp))
                                numSpots++;
    ...
    private void _demoteSomethingTypeless(final GPNode node, final EvolutionState state, final int thread, final GPFunctionSet set) throws CloneNotSupportedException
        {
        int numDemotable = 0;
    ...
    private int numDemotableNodes(final GPNode root, int soFar, final GPFunctionSet set)
        {
        ...
        // otherwise, I gotta do the dirty work
        else return _numDemotableNodes(root,soFar,set);
        }


    private int _numDemotableNodes(final GPNode root, int soFar, final GPFunctionSet set)
        {
        if (demotable(root, set)) soFar++;
        for(int x=0;x<root.children.length;x++) 
            soFar = _numDemotableNodes(root.children[x],soFar, set);
        return soFar;
        }
    ...
    private int pickDemotableNode(final GPNode root, int num, final GPFunctionSet set)
        {
        ...
        // otherwise, I gotta do the dirty work
        else return _pickDemotableNode(root,num,set);
        }
    

    // sticks the node in 
    private int _pickDemotableNode(final GPNode root, int num, final GPFunctionSet set)
        {
        if (demotable(root, set))
            {
            num--;
            if (num==-1)  // found it
                {
        ...
        for(int x=0;x<root.children.length;x++)
            {
            num = _pickDemotableNode(root.children[x],num,set);
    ...
    public int produce(final int min, 
        ...
        for(int q=start; q < n+start; q++)
            ...
            for (int x=0;x<numTries;x++)
                ...
                int numdemote = numDemotableNodes(j.trees[t].child,0,j.trees[t].constraints().functionset);
                if (numdemote==0) continue; // uh oh, try again
                
                // demote the node, or if we're unsuccessful, just leave it alone
                pickDemotableNode(j.trees[t].child,state.random[thread].nextInt(numdemote),j.trees[t].constraints().functionset);
                
                // does this node exceed the maximum depth limits?
                if (!verifyPoint(demotableNode)) continue; // uh oh, try again
                
                // demote it
                demoteSomething(demotableNode,state,thread,j.trees[t].constraints().functionset);
                j.evaluated = false;
                }
