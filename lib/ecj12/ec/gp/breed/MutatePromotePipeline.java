...
public class MutatePromotePipeline extends GPBreedingPipeline
    ...
    private boolean promotable(final GPNode node)
        {
        ...
        if (parent.parent instanceof GPNode)  // ugh, expensive
            t = ((GPNode)(parent.parent)).constraints().childtypes[parent.argposition];
        else 
            t = ((GPTree)(parent.parent)).constraints().treetype;

        // 2: the node's returntype is type-compatible with its GRANDparent's return slot
        return (node.constraints().returntype.compatibleWith(t));
        }
    ...
    private int numPromotableNodes(final GPNode root, int soFar)
        {
        if (promotable(root)) soFar++;
        for(int x=0;x<root.children.length;x++) 
            soFar = numPromotableNodes(root.children[x],soFar);
        return soFar;
        }
    ...
    // sticks the node in 
    private int pickPromotableNode(final GPNode root, int num)
        {
        if (promotable(root))
            {
            num--;
        ...
        for(int x=0;x<root.children.length;x++)
            {
            num = pickPromotableNode(root.children[x],num);
    ...
    public int produce(final int min, 
        ...
        for(int q=start; q < n+start; q++)
            ...
            for (int x=0;x<numTries;x++)
                ...
                int numpromote = numPromotableNodes(j.trees[t].child,0);
                if (numpromote==0) continue; // uh oh, try again
                
                // promote the node, or if we're unsuccessful, just leave it alone
                pickPromotableNode(j.trees[t].child,state.random[thread].
                                   nextInt(numpromote));
