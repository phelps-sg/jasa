...
public class MutateAllNodesPipeline extends GPBreedingPipeline
    ...
    public Object protoClone() throws CloneNotSupportedException
        {
        MutateAllNodesPipeline c = (MutateAllNodesPipeline)(super.protoClone());
        
        // deep-cloned stuff
        c.nodeselect = (GPNodeSelector)(nodeselect.protoClone());
        return c;
        }
    ...
    private GPNode pickCompatibleNode(final GPNode original, final GPFunctionSet set, final EvolutionState state, final GPType returntype, final int thread)
        ...
        int len = original.constraints().childtypes.length;
        boolean failed;

        if (GPType.numAtomicTypes + GPType.numSetTypes == 1)  // easy
            ...
            for(int y=0;y<set.nodesByArity[type][len][x].
                    node.constraints().childtypes.length;y++)
                if (!set.nodesByArity[type][len][x].node.constraints().
                    childtypes[y].compatibleWith(original.children[y].
    ...
    private GPNode generateCompatibleTree(final GPNode original, final GPFunctionSet set, final EvolutionState state, final GPType returntype, final int thread) throws CloneNotSupportedException
        {
        // pick a new node and clone it
        GPNode node = (GPNode)(pickCompatibleNode(original,set,state,returntype,thread).protoClone());
        ...
        for (int x=0;x<node.children.length;x++)
            {
            node.children[x] = generateCompatibleTree(original.children[x],set,state,original.constraints().childtypes[x],thread);
    ...
    public int produce(final int min, 
        ...
        for(int q=start; q < n+start; q++)
            ...
            p2 = generateCompatibleTree(p1,i.trees[t].constraints().functionset,state,type,thread);
