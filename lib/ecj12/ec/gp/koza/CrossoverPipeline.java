...
public class CrossoverPipeline extends GPBreedingPipeline
    ...
    public Object protoClone() throws CloneNotSupportedException
        {
        CrossoverPipeline c = (CrossoverPipeline)(super.protoClone());

        // deep-cloned stuff
        c.nodeselect1 = (GPNodeSelector)(nodeselect1.protoClone());
        c.nodeselect2 = (GPNodeSelector)(nodeselect2.protoClone());
    ...
    public void setup(final EvolutionState state, final Parameter base)
        ...
        if (state.parameters.exists(p,d) &&
            ...
            nodeselect2 = (GPNodeSelector)(nodeselect1.protoCloneSimple());
        else
            {
    ...
    public final boolean verifyPoints(GPNode inner1, GPNode inner2)
        ...
        // on a type basis
        if (!inner1.swapCompatibleWith(inner2)) return false;
        ...
        for(int q=start;q<n+start; /* no increment */)  // keep on going until we're filled up
            ...
            if (tree1==TREE_UNFIXED || tree2==TREE_UNFIXED) 
                ...
                do
                    ...
                    } while (parents[0].trees[t1].constraints() != parents[1].trees[t2].constraints());
                }
            else
                {
                t1 = tree1;
                ...
                if (parents[0].trees[t1].constraints() 
                    != parents[1].trees[t2].constraints()) // uh oh
            ...
            for(int x=0;x<numTries;x++)
                ...
                res1 = verifyPoints(p2,p1);  // p2 can fill p1's spot -- order is important!
                if (n-(q-start)<2 || tossSecondParent) res2 = true;
                else res2 = verifyPoints(p1,p2);  // p1 can fill p2's spot -- order is important!
            ...
            GPIndividual j1 = (GPIndividual)(parents[0].protoClone());
            GPIndividual j2 = null;
            if (n-(q-start)>=2 && !tossSecondParent) j2 = (GPIndividual)(parents[1].protoClone());
            ...
            for(int x=0;x<j1.trees.length;x++)
                ...
                if (x==t1 && res1)  // we've got a tree with a kicking cross position!
                    { 
                    j1.trees[x] = (GPTree)(parents[0].trees[x].protoClone());
            ...
            if (n-(q-start)>=2 && !tossSecondParent) for(int x=0;x<j2.trees.length;x++)
                ...
                if (x==t2 && res2)  // we've got a tree with a kicking cross position!
                    { 
                    j2.trees[x] = (GPTree)(parents[1].trees[x].protoClone());           
