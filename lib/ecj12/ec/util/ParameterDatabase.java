 ...
 * parents. 
 *
 * <p>An optional set of parameters, "print-params", specifies whether or not
 * parameters should be printed as they are used (through one of the get(...) methods).
 * If print-params is unset, or set to false or FALSE, nothing is printed.
 * If set to non-faslse, then the parameters are printed.
 *
 * <p>
...
public final class ParameterDatabase extends Properties implements Serializable
    ...
    public static final int PS_PRINT_GOTTEN = 1;
    ...
    protected final int parseInt(String string) throws NumberFormatException
        {
        char c;
    ...
    // we assume that the string has been trimmed already
    protected final long parseLong(String string) throws NumberFormatException
        {
        char c;
        if (string!=null && string.length() > 0 && ((string.charAt(0)==(c='x')) || c == 'X'))
            {
    ...
    public final int getInt(Parameter parameter) throws NumberFormatException
        {
        if (exists(parameter))
            {
            try
            ...
            catch(NumberFormatException e) { throw new NumberFormatException("Bad integer ("
                                                                             +get(parameter) + " ) for parameter " + parameter); }
            }
        else throw new NumberFormatException("Integer does not exist for parameter " + parameter);
        }

    /** Searches down through databases to find a given parameter, whose value must be an integer.  It returns the value, else throws a NumberFormatException exception if there is an error in parsing the parameter. The parameter chosen is marked "used" if it exists. Integers may be in decimal or (if preceded with an X or x) in hexadecimal.  */
    public final int getInt(Parameter parameter, Parameter defaultParameter) throws NumberFormatException
        {
        if (exists(parameter)) return getInt(parameter);
        else if (exists(defaultParameter)) return getInt(defaultParameter);
        else throw new NumberFormatException("Integer does not exist for either parameter " 
                                             + parameter + "\nor\n" + defaultParameter);
        }
    ...
    final int getIntWithDefault(Parameter parameter, int defaultValue)
        ...
        if (exists(parameter))
            {
            try { return parseInt(get(parameter)); }
    ...
    public final float getFloatWithDefault(Parameter parameter, Parameter defaultParameter, double defaultValue)
        {
        if (exists(parameter)) return getFloatWithDefault(parameter,defaultValue);
        else return getFloatWithDefault(defaultParameter, defaultValue);
        }
    ...
    final float getFloatWithDefault(Parameter parameter, double defaultValue)
        ...
        if (exists(parameter))
            ...
            catch(NumberFormatException e) { return (float)(defaultValue); }
            }
    ...
    public final double getDouble(Parameter parameter, Parameter defaultParameter, double minValue)
        {
        if (exists(parameter)) return getDouble(parameter,minValue);
        else return getDouble(defaultParameter, minValue);
        }
    ...
    final double getDouble(Parameter parameter, double minValue)
        ...
        if (exists(parameter))
            ...
            try
                ...
                if (i < minValue) return (double)(minValue-1);
                return i;
                }
            catch(NumberFormatException e) { return (double)(minValue-1); }
            }
        else return (double)(minValue-1);
        }
    ...
    public final double getDouble(Parameter parameter, Parameter defaultParameter, double minValue, double maxValue)
        {
        if (exists(parameter)) return getDouble(parameter,minValue,maxValue);
        else return getDouble(defaultParameter,minValue,maxValue);
        }
    ...
    final double getDouble(Parameter parameter, double minValue, double maxValue)
        ...
        if (exists(parameter))
            ...
            try
                ...
                if (i < minValue) return (double)(minValue-1);
                if (i > maxValue) return (double)(minValue-1);
                return i;
                }
            catch(NumberFormatException e) { return (double)(minValue-1); }
            }
    ...
    public final double getDoubleWithDefault(Parameter parameter, Parameter defaultParameter, double defaultValue)
        {
        if (exists(parameter)) return getDoubleWithDefault(parameter,defaultValue);
        else return getDoubleWithDefault(defaultParameter, defaultValue);
        }
    ...
    public final long getLong(Parameter parameter) throws NumberFormatException
        {
        if (exists(parameter))
            {
            try
            ...
            catch(NumberFormatException e) { throw new NumberFormatException("Bad long ("
                                                                             +get(parameter) + " ) for parameter " + parameter); }
            }
        else throw new NumberFormatException("Long does not exist for parameter " + parameter);
        }

    /** Searches down through databases to find a given parameter, whose value must be a long.  It returns the value, else throws a NumberFormatException exception if there is an error in parsing the parameter. The parameter chosen is marked "used" if it exists. Longs may be in decimal or (if preceded with an X or x) in hexadecimal.  */
    public final long getLong(Parameter parameter, Parameter defaultParameter) throws NumberFormatException
        {
        if (exists(parameter)) return getLong(parameter);
        else if (exists(defaultParameter)) return getLong(defaultParameter);
        else throw new NumberFormatException("Long does not exist for either parameter " 
                                             + parameter + "\nor\n" + defaultParameter);
        }


    /** Searches down through databases to find a given parameter, whose value must be a long >= minValue.  
        If not, this method returns errValue, else it returns the parameter value. The parameter chosen is marked "used" if it exists.  Longs may be in decimal or (if preceded with an X or x) in hexadecimal.*/
    ...
    public final long getLongWithDefault(Parameter parameter, Parameter defaultParameter, long defaultValue)
        {
        if (exists(parameter)) return getLongWithDefault(parameter, defaultValue);
        else return getLongWithDefault(defaultParameter, defaultValue);
        }
    ...
    final long getLongWithDefault(Parameter parameter, long defaultValue)
        ...
        if (exists(parameter))
            {
            try { return parseLong(get(parameter)); }
            catch(NumberFormatException e) { return defaultValue; }
            }
        else return defaultValue;
        }
    ...
    */
    ...
    /** Use getLongWithMax(...) instead.  Searches down through databases to find a given parameter, whose value must be a long >= minValue and =< maxValue.  If not, this method returns errValue, else it returns the parameter value. The parameter chosen is marked "used" if it exists. 
        @deprecated
    */
    ...
    protected final synchronized String get(Parameter parameter)
        ...
        if (printState==PS_UNKNOWN)
            try { 
                String jp = _get(PRINT_PARAMS);
                if (jp==null || jp.equalsIgnoreCase("false"))
                    printState=PS_NONE;
                else printState=PS_PRINT_GOTTEN;
                }
            catch (NumberFormatException e)
                { printState = PS_NONE; }
        uncheck();
        
        if (printState==PS_PRINT_GOTTEN)
    ...
    */
    ...
    public final void list(PrintStream p, boolean listShadowed) { list(new PrintWriter(p), listShadowed); }

    /** Prints out all the parameters in the database, but not shadowed parameters. */
    public final void list(PrintStream p) { list(new PrintWriter(p),false); }

    /** Prints out all the parameters in the database, but not shadowed parameters. */
    public final void list(PrintWriter p) { list(p,false); }
