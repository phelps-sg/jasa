...
public class IslandExchange extends Exchanger
    ...
    // PrintWriters to the destination islands
    PrintWriter[] outWriters;
...
//    int mailboxCapacity;
    ...
    public static void main(String[] args) throws InterruptedException
        ...
        EvolutionState myEvolutionState = new
            EvolutionState() { public void run(int condition) { }};
    ...
    public void initializeContacts(EvolutionState state)
        ...
        try
            ...
            fromServer = new LineNumberReader(new InputStreamReader(serverSocket.getInputStream()));
            toServer = new PrintWriter(serverSocket.getOutputStream(), true);

            // sending the server own contact information
            toServer.println( ownId );
            ...
            mailbox = new IslandExchangeMailbox( state, clientPort, Integer.parseInt( fromServer.readLine() ),
                                                 Integer.parseInt( fromServer.readLine() ) , ownId, chatty, compressedCommunication );
            ...
            try
                {
                toServer.println( InetAddress.getLocalHost().getHostAddress() );
            ...
            toServer.println( mailbox.getPort() );
            ...
            synchronous = ( Integer.parseInt( fromServer.readLine() ) == 1 );
            if( synchronous )
                {
            ...
            modulo = Integer.parseInt( fromServer.readLine() );
            offset = Integer.parseInt( fromServer.readLine() );
            size = Integer.parseInt( fromServer.readLine() );

            // read the number of islands it has to send messages to
            number_of_destination_islands = Integer.parseInt( fromServer.readLine() );

            // allocate the arrays
            outSockets = new Socket[ number_of_destination_islands ];
            outWriters = new PrintWriter[ number_of_destination_islands ];
            ...
            for( int y = 0 ; y < number_of_destination_islands ; y++ )
                ...
                String address = fromServer.readLine().trim();
                int port = Integer.parseInt( fromServer.readLine() );
                try
                    {
                    try
                        {
                    ...
                    if( compressedCommunication )
                        {
                        outWriters[y] = new PrintWriter( new CompressingOutputStream(outSockets[y].getOutputStream()), true );

                        // read the mailbox's id, then write my own id
                        outgoingIds[y] = new LineNumberReader(new InputStreamReader(
                                                                  new CompressingInputStream(outSockets[y].getInputStream()))).readLine().trim();
                        }
                    else
                        {
                        outWriters[y] = new PrintWriter( outSockets[y].getOutputStream(), true );

                        // read the mailbox's id, then write my own id
                        outgoingIds[y] = new LineNumberReader(new InputStreamReader(
                                                                  outSockets[y].getInputStream())).readLine().trim();
                        }
                        
                    outWriters[y].println(ownId);
        ...
        if( ( state.generation >= offset ) &&
            ...
            for( int x = 0 ; x < number_of_destination_islands ; x++ )
                ...
                if( running[x] )
                    ...
                    if (chatty) state.output.message( "Sending the emigrants to island " + outgoingIds[x] );
    ...
    public String runComplete(EvolutionState state)
        ...
        try
            ...
            state.output.warning( "Some weird IO exception reported by the system in the IslandExchange::runComplete function." );
            }
    ...
    public void closeContacts(EvolutionState state, int result)
        ...
        if( result == state.R_SUCCESS )
            {
            toServer.println( FOUND );
            }
...
class IslandExchangeMailbox implements Runnable
    ...
    */
    public IslandExchangeMailbox( final EvolutionState state_p, int port, int n_incoming_p, int how_many, String _myId, boolean chatty, boolean _compressedCommunication )
        ...
        inReaders = new LineNumberReader[ n_incoming ];
    ...
    public void run()
        ...
        for( int x = 0 ; x < n_incoming ; x++ )
            ...
            try
                ...
                if( compressedCommunication )
                    {
                    inReaders[x] = new LineNumberReader( new InputStreamReader
                                                         (new CompressingInputStream(inSockets[x].getInputStream())) );
                        
                    // send my id, then read an id
                    printer = new PrintWriter(new CompressingOutputStream(inSockets[x].getOutputStream()), true);
                    }
                else
                    {
                    inReaders[x] = new LineNumberReader( new InputStreamReader
                                                         (inSockets[x].getInputStream()) );
                        
                    // send my id, then read an id
                    printer = new PrintWriter(inSockets[x].getOutputStream(), true);
                    }
                printer.println(myId);
                incomingIds[x] = inReaders[x].readLine().trim();    
                ...
                if( running[x] )
                    ...
                    try
                        ...
                        while( true )
                            ...
                            int subpop = Integer.parseInt( inReaders[x].readLine() );
                            ...
                            int how_many_to_come = Integer.parseInt( inReaders[x].readLine() );
                                    ...
                                    try
                                        ...
                                        immigrants[subpop][person2die[subpop]] = state.population.subpops[subpop].species.
                                            newIndividual( state, state.population.subpops[subpop],
                                                           (Fitness)state.population.subpops[subpop].f_prototype.protoCloneSimple(),
                                                           inReaders[x] );
                                        ...
                                        {
                                        // will never happen
                                        state.output.message( "CloneNotSupported exception while communicating with an island" );
                                        running[x] = false;
                                        continue;
                                        }
                                    catch( IOException e )
...
/** A class indicating all the information the server knows about
    ...
    public void run()
        ...
        LineNumberReader[] in = new LineNumberReader[numIslands];
        PrintWriter[] out = new PrintWriter[numIslands];

        // IDs of the islands
        String[] requestedIDs = new String[numIslands];
        ...
        for(int x=0;x<numIslands;x++)
            ...
            try
                ...
                in[x] = new LineNumberReader(new InputStreamReader(con[x].getInputStream()));
                out[x] = new PrintWriter(con[x].getOutputStream(), true);

                // read the id
                connected_island_ids[x] = in[x].readLine().trim();
                ...
                out[x].println( ieii.num_incoming );

                // send the capacity of the mailbox
                out[x].println( ieii.mailbox_capacity );

                // read the address and port of the island
                ieii.address = in[x].readLine().trim();
                ieii.port = Integer.parseInt(in[x].readLine());
        ...
        for( int x = 0 ; x < numIslands ; x++ )
            ...
            if( clientRunning[x] )
                ...
                if( synchronous )
                    ...
                    out[x].println( 0 );
                out[x].println( ieii.modulo );
                out[x].println( ieii.offset );
                out[x].println( ieii.size );

                // send the number of address-port pairs that will be sent
                out[x].println( ieii.num_mig );
                ...
                for( int y = 0 ; y < ieii.num_mig ; y++ )
                    ...
                    if( temp == null )
                        ...
                        out[x].println( temp.address );
                        out[x].println( temp.port );
                        }
                    }
                }
            }
