    ...
    public Population breedPopulation(EvolutionState state) throws CloneNotSupportedException
        ...
        for(int x=0;x<state.population.subpops.length;x++)
            if (elite[x]>state.population.subpops[x].individuals.length)
                state.output.error("The number of elites for subpopulation " + x + " exceeds the actual size of the subpopulation", new Parameter(state.P_BREEDER).push(P_ELITE).push(""+x));
        state.output.exitIfErrors();
    ...
    public void loadElites(EvolutionState state, Population newpop)
        ...
        for(int sub=0;sub<state.population.subpops.length;sub++) 
            ...
            if (elite[sub]==1)
                ...
                inds[inds.length-1] = (Individual)(oldinds[best].deepClone());
                }
                ...
                for(int x=inds.length-elite[sub];x<inds.length;x++)
                    inds[x] = (Individual)(oldinds[orderedPop[x]].deepClone());
                }
        }
    }
