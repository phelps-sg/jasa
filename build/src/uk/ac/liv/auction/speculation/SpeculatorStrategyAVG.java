 ...
 *
 */
public class SpeculatorStrategyAVG	extends FixedQuantityStrategyImpl
	...
	public SpeculatorStrategyAVG( AbstractTradingAgent agent) {
	  ...
	  public boolean modifyShout ( Shout.MutableShout shout) {
		  ...
		  // retrieve current equilibrium price (taken from uk.ac.liv.auction.agent.EquilibriumPriceStrategy)
		  EquilibriumReport eqReport = new EquilibriumReport((RoundRobinAuction) auction);
		  eqReport.calculate();
		  double currentPrice = eqReport.calculateMidEquilibriumPrice();
		  
		  // update forecasted price
		  forecastedPrice = forecastedPriceCoefficient * forecastedPrice + (1-forecastedPriceCoefficient) * currentPrice;
	  ...
	  public void endOfRound ( Auction auction) {
		  //update forecasted equilibrium price (taken from uk.ac.liv.auction.agent.EquilibriumPriceStrategy)
		  EquilibriumReport eqReport = new EquilibriumReport((RoundRobinAuction) auction);
		  eqReport.calculate();
