...
public class RepastSupplyChainSimulation extends SimModelImpl implements
  ...
  public void setupAuctions( ParameterDatabase parameters, Parameter base ) {
	  ...
	  /*
	   * comming from SupplyChainSimulation, but not used in RepastMarketSimulation:
	   *
  ...
  public void setupAgents( ParameterDatabase parameters, Parameter base ) {
	  ...
	  for (int t = 0; t < numAgentTypes; t++) {
		  ...
		  for (int i = 0; i < numAgents; i++) {
			  ...
			  //((uk.ac.liv.supplyChain.SupplyChainAgent) agent).printSetup();
...
//TODO
/*	  Iterator wewe = MyGraphReport.getSingletonInstance().getSequenceIterator();
	  RepastGraphSequence sequenceAsk = (RepastGraphSequence) wewe.next();
	  double ask = sequenceAsk.getSValue();
	  System.out.println("RepastSupplyChainSimulation.step: Mean ask quote = "+ask);
	  RepastGraphSequence sequenceBid = (RepastGraphSequence) wewe.next();
	  double bid = sequenceAsk.getSValue();
	  System.out.println("RepastSupplyChainSimulation.step: Mean bid quote = "+bid);
	  RepastGraphSequence sequenceTrans = (RepastGraphSequence) wewe.next();
	  System.out.println("RepastSupplyChainSimulation.step: Mean trans quote = "+sequenceTrans.getSValue());
	  //auction[0].setTransactionPrice( (ask + bid) / 2 );
*/
//TODO
	  if (display) {
		  System.out.println("\n**********************"+auction[0].getAge()+" RepastSupplyChainSimulation.step Auction "+auction[0].getId()+ " previous transaction prices:");
		  DecimalFormat df = new DecimalFormat("########.0000");
		  for ( int i=0 ; i<auction.length ; i++) {
			  System.out.print("[Auc"+i+": ");
			  for ( int j=0 ; j<(int)auction[0].getAge()+1 ; j++)
				  System.out.print(  ((SupplyChainRandomRobinAuction) auction[i]).getTransactionPrice( j ) +",");
			  //System.out.print( df.format( ((SupplyChainRandomRobinAuction) auction[i]).getTransactionPrice( j ) )+",");
			  System.out.println("] ");
		  }
		  System.out.println("**********************");
	  }
	  ...
	  try {
		  for (int a = 0; a < auction.length; a++ ) {
			  if (display)
				  System.out.println("*********"+auction[0].getAge()+" RepastSupplyChainSimulation.step Auction "+auction[a].getId()+"*********");
			  auction[a].step();
		  }
  ...
  protected void buildDisplay() {
	  ...
	  for ( int i=0 ; i < auction.length ; i++ ) {
		  //if ( (graphLogger = MyGraphReport.getInstance(i)) != null ) {
  ...
  class AgentMatrix implements BaseMatrix {
    ...
    public AgentMatrix( ArrayList agents, int height, int width, RandomRobinAuction auction ) {
