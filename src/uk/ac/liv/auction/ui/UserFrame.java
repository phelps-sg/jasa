...
public class UserFrame extends JFrame implements Parameterizable {
	...
	public void setup(ParameterDatabase parameters, Parameter base) {
		...
		setTitle(parameters.getStringWithDefault(base.push(P_TITLE), null, ""));

		Font font = ObjectConverter.getFont(parameters.getString(base.push(P_FONT),
		    null));
		if (font != null)
			setFont(font);

		Color color = ObjectConverter.getColor(parameters.getString(base
		    .push(P_BACKGROUND), null));
		if (color != null)
			setBackground(color);

		color = ObjectConverter.getColor(parameters.getString(base
		    .push(P_FOREGROUND), null));
		if (color != null)
			setForeground(color);
