 ...
 *
 */
public class SimpleTradingAgent extends AbstractTradingAgent implements Serializable {
	...
	/* (non-Javadoc)
	 * @see uk.ac.liv.auction.agent.AbstractTradingAgent#equilibriumProfits(uk.ac.liv.auction.core.Auction, double, int)
	 */
	public double equilibriumProfits( Auction auction, double equilibriumPrice, int quantity ) {
	...
	public void requestShout( Auction auction ) {
		//if (getId()==0) System.out.println("DAY in requestShout="+auction.getDay());
		...
		if (getStock() > reserve) {
			isSeller = false;
			//if(getId() == 2) System.out.println("Agent "+getId()+" tries to buy");
		}//if
		
		if (getStock() < reserve) {
			isSeller = true;
			//if(getId() == 2) System.out.println("Agent "+getId()+" tries to sell");
		}//if
	...
	public void setup( ParameterDatabase parameters, Parameter base ) {
		...
		b00	= parameters.getInt(base.push(P_B00));
		b01	= parameters.getInt(base.push(P_B01));
		b0inf	= parameters.getInt(base.push(P_B0INF));
		
		int min	= parameters.getInt(base.push(P_SKILL_FOOD_MIN));
		int max	= parameters.getInt(base.push(P_SKILL_FOOD_MAX));
		skillF	= Random.uniform.nextIntFromTo(min, max);
	
		min	= parameters.getInt(base.push(P_SKILL_GOLD_MIN));
		max	= parameters.getInt(base.push(P_SKILL_GOLD_MAX));
		skillG	= Random.uniform.nextIntFromTo(min, max); 

		min	= parameters.getInt(base.push(P_RESERVE_MIN));
		max	= parameters.getInt(base.push(P_RESERVE_MAX));
	...
	public double updateEquilibriumPrice(Auction auction) {
		...
		//System.out.println("simpleTradingAgent.updateEquilibriumPrice: new price for agent "+getId()+"="+previousEqPrice);
		//System.out.println(toString(auction));
		return previousEqPrice;
	}//updateEquilirbiumPrice
	
	
	public void informOfSeller( Auction auction, Shout winningShout, TradingAgent seller, double price, int quantity) {
		//TODO
		...
		if (((SimpleTradingAgent)seller).acceptDeal(auction, price, quantity)) {
			//TODO
			//purchaseFrom(auction, (SimpleTradingAgent) seller, quantity, price);
			//if(getId() == 2) System.out.println("Agent "+getId()+" buys "+quantity+" items from agent "+((SimpleTradingAgent)seller).getId()+" for $"+price+"/item.");
		} else { //this else shouldn't occur
			assert ((SimpleTradingAgent)seller).acceptDeal(auction, price, quantity);
			//if(getId() == 2) System.out.println("Agent "+getId()+" REJECTS to buy "+quantity+" items from agent "+((SimpleTradingAgent)seller).getId()+" for $"+price+"/item.");
		}
	}
	
	public boolean acceptDeal( Auction auction, double price, int quantity ) {
		assert isSeller;
		//if (getId()==2) System.out.println("acceptDeal"+ isSeller + "\tprice="+price+"bid="+valuer.determineValue(auction));
		return price >= valuer.determineValue(auction);
	}
	
	public String toString() {
		return new String("Ag"+getId()+": [FOOD="+getStock()+" skF="+skillF+" res="+reserve+"]  [ GOLD="+getFunds()+" skG="+skillG+"] eqPrice="+previousEqPrice);
	}
	
	public String toString(Auction auction) {
		return	"Day" + Integer.toString(auction.getDay())
				+ "R" + Integer.toString(auction.getRound())
				+ " " + toString()
				+ "\tbid="+valuer.determineValue(auction);
	}
	
	public void endOfDay( AuctionEvent event ) {
		
		//updateEquilibriumPrice() is called at the end of the day, so that _PreviousPriceValuer can use the previous price.
		Auction auction = (Auction)event.getAuction();
		updateEquilibriumPrice(auction);
		
		//if (getId()==0) System.out.println("DAY in endOfDay="+auction.getDay());
	...
	public void produce() {
		/** MBC book, p.4: agent produces gold if and only if
		 *  skillG > previousEqPrice.skillF	(equation1)
		 *  else, food is produced
		 */
		...
		//if (skillG > previousEqPrice * skillF  &&  stock > dailyConsumption) {
			if(true){
				//TODO
			//funds += skillG;
			//if(getId() == 2) System.out.println("Agent "+getId()+" produces "+skillG+ " units of gold (food="+stock+"/gold="+funds+"/price="+previousEqPrice+")");
		}
		else {
			stock.add((int)skillF);
			//if(getId() == 2) System.out.println("Agent "+getId()+" produces "+skillF+ " units of food (food="+stock+"/gold="+funds+"/price="+previousEqPrice+")");
		}
		
		if (getStock()<0) {
			//System.out.println("Agent "+getId()+" has a negative inventory of " + getStock());
		}
		if (getFunds()<0) {
			//System.out.println("Agent "+getId()+" has a negative funds of " + getFunds());
		}
	}
