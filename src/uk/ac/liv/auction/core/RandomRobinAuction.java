	...
	public void setup(ParameterDatabase parameters, Parameter base) {
		...
		try {
			closingCondition = (TimingCondition) parameters.getInstanceForParameter(
			    base.push(P_AUCTION_CLOSING), defBase.push(P_AUCTION_CLOSING),
		...
		try {
			dayEndingCondition = (TimingCondition) parameters
			    .getInstanceForParameter(base.push(P_DAY_ENDING), defBase
			        .push(P_DAY_ENDING), DayEndingCondition.class);
		...
		try {
			report = (AuctionReport) parameters.getInstanceForParameter(base
			    .push(P_REPORT), defBase.push(P_REPORT), AuctionReport.class);
		...
		if (parameters.getBoolean(base.push(P_CONSOLE), defBase.push(P_CONSOLE),
		    false)) {
			activateGUIConsole();
		}
		...
		try {
			AuctionEventListener eventHandler = (AuctionEventListener) parameters
			    .getInstanceForParameter(base.push(P_EVENTHANDLER), defBase
			        .push(P_EVENTHANDLER), AuctionEventListener.class);
