...
*/
	  ...
	  /*	  if (true) {
		  ...
		  for ( int i=0 ; i<auction.length ; i++) {
			  System.out.print("[Auc"+i+" current: ");
	  ...
	  try {
		  for (int a = 0; a < auction.length; a++ ) {
			  if (display)
				  System.out.println("\n*********"+auction[0].getAge()+" RepastSupplyChainSimulation.step Auction "+auction[a].getId()+"*********");
			  auction[a].step();
		  }
 ...
 */
  private void printClearingPrices() {
	  ...
	  for ( int j=0 ; j<(int)auction[0].getAge()+1 ; j++){
		  System.out.print( "\n" + j );
		  for ( int i=0 ; i<auction.length ; i++)
			  System.out.print( "\t" + i+ "\t" + df.format( ((SupplyChainRandomRobinAuction) auction[i]).getTransactionPrice( j ) ) );
			  //System.out.print(  ((SupplyChainRandomRobinAuction) auction[i]).getTransactionPrice( j ) +"\t");
	  }
  }
