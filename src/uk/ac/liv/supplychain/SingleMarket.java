...
//			RectNetworkItem drawable = new RectNetworkItem(x, y);
		...
		/*
		 * //link every market to every retailer for (int retailerIndex = 1 ;
		 * retailerIndex < numRetailers+1 ; retailerIndex++) { Company retailer =
		 * (Company)agentList.get(retailerIndex); CompanyLink link = new
		 * CompanyLink(market, retailer); market.addOutEdge(link); }//for
		 */
