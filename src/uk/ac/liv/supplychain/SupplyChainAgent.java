...
public abstract class SupplyChainAgent extends uk.ac.liv.auction.zi.ZITraderAgent {
	...
	public void setup( ParameterDatabase parameters, Parameter base, SupplyChainRandomRobinAuction[] auction ) {
	    ...
	    for (int i=0 ; i < source_types ; i++) {
	        ...
	        try { // set source[i], either explicitly given in the config file, or randomly drawn between min and max
	        	String[] tempSource			= (parameters.getString(defTypeParamT.push(P_SOURCE_INITIAL))).split(";");
	        	if ( tempSource.length == 1)
	        		...
	        		source[i]				= (int)sourceMin + (int) ( (sourceMax - sourceMin) * distribution.nextDouble()  );
	        	} else
	        		System.out.println("ERROR: "+defTypeParamT.push(P_SOURCE_INITIAL) + " has too many parameters.");
	        } catch(java.lang.NullPointerException e)
	        {
	        	System.out.println("ERROR: "+defTypeParamT.push(P_SOURCE_INITIAL) + " needs at least one parameter of type double.");
	        	throw e;
	        }
	    ...
	    for (int i=0 ; i < deliver_types ; i++) {
	        ...
	        try { // set deliver[i], either explicitly given in the config file, or randomly drawn between min and max
	        	String[] tempDeliver	= (parameters.getString(defTypeParamT.push(P_DELIVER_INITIAL))).split(";");
	        	if ( tempDeliver.length == 1 )
	        		...
	        		deliver[i]				= (int)deliverMin + (int) ( (deliverMax - deliverMin) * distribution.nextDouble()  );
	        	} else
	        		System.out.println("ERROR: "+defTypeParamT.push(P_DELIVER_INITIAL) + " has too many parameters.");
	        } catch(java.lang.NullPointerException e) {
	        	System.out.println("ERROR: "+defTypeParamT.push(P_DELIVER_INITIAL)+ " needs at least one parameter of type double.");
	        	throw e;
	        }
	...
	public void shoutAccepted( Auction auction, Shout shout, double price, int quantity ) {
		...
		if (display) {
			System.out.print(auction.getAge()+" SupplyChainAgent.shoutAccepted: "+getClass().toString()+ " " + getId()+ " qty="+ quantity+" at $"+price+" in Auct "+auctionIdx);
			if (shout.isAsk()) {
				System.out.println(" GIVES "+quantity+" at $"+price+" to take from deliver"+deliverWhichSellsInAuction[auctionIdx]);
			}
			if (shout.isBid()) {
				System.out.println(" RECEIVES "+quantity+" at $"+price+" to put into source"+sourceWhichBuysInAuction[auctionIdx]);
			}
		}
		
		//System.out.println("auctionForWhichThisSourceBuys[ sourIdx ]="+auctionForWhichThisSourceBuys[ 0 ]+" auctionIdx="+auctionIdx);
		...
		for ( int sourIdx = 0 ; sourIdx < source.length ;  sourIdx++ ) {
			if ( ( shout.isAsk() ) && ( auctionForWhichThisSourceSells[ sourIdx ] == auctionIdx ) ) {
			...
			else if ( ( shout.isBid() ) && ( auctionForWhichThisSourceBuys[ sourIdx ] == auctionIdx ) ){
				source[ sourIdx ] += quantity;
				//TODO (quantity-1) (instead of quantity) was necessary because RandomRobinAuction.clear() transfers all products but pays for only one unit
				account.credit( - quantity * price );
				if (display)
		...
		for ( int delIdx = 0 ; delIdx < deliver.length ; delIdx++ ) {
			if ( ( shout.isAsk() ) && ( auctionForWhichThisDeliverSells[ delIdx ] == auctionIdx ) ) {
			...
			else if ( ( shout.isBid() ) && ( auctionForWhichThisDeliverBuys[ delIdx ] == auctionIdx )
					&& ( shout.isBid() ) ) {
				deliver[ delIdx ] += quantity;
				//TODO (quantity-1) (instead of quantity) was necessary because RandomRobinAuction.clear() transfers all products but pays for only one unit
				account.credit( - quantity * price );
	...
	public void _shoutAccepted( Auction auction, Shout shout, double price, int quantity ) {
		int auctionIdx			= (int) ((SupplyChainRandomRobinAuction) auction).getId();

		System.out.print("\n"+auction.getAge()+" SupplyChainAgent.shoutAccepted: agent "+getId()+ " qty="+ quantity+" at $"+price+" in Auct "+auctionIdx);
		if (shout.isAsk())
