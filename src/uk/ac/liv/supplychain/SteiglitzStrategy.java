...
//		    .getValuationPolicy();
		...
		if (display)
			System.out.println(auction.getAge()
			    + " BEGINning of SteiglitzStrategy.modifyShout auction=" + auctionIdx
			    + " agent=" + agent.getId() + "    ***  AGENT = " + agent.toString());
		...
		for (int sourIdx = 0; sourIdx < agent.source.length; sourIdx++) {
			if (agent.auctionForWhichThisSourceSells[sourIdx] == auctionIdx)
				if (agent.source[sourIdx] > agent.source_capacity[sourIdx]) {
					...
					shout
					    .setQuantity((int) (-agent.source_capacity[sourIdx] + agent.source[sourIdx]));
					toReturn = true;
					if (display)
						System.out.println("case 1 source" + sourIdx + " auction="
						    + agent.auctionForWhichThisSourceSells[sourIdx] + " agent="
						    + agent.getId() + " ");
				}
			...
			if (agent.auctionForWhichThisSourceBuys[sourIdx] == auctionIdx)
				if (agent.source[sourIdx] <= agent.source_capacity[sourIdx]) {
					...
					if (agent.getFunds() > qty2buy * valuation) {
						...
						if (display)
							System.out.println("case 21 agent.getFunds()=" + agent.getFunds()
							    + " > " + qty2buy + " * valuation=" + valuation + " = "
							    + (qty2buy * valuation) + " => shout.getQuantity()="
							    + shout.getQuantity());
					} else {
						shout.setQuantity((int) (agent.getFunds() / valuation));
						if (display)
							System.out.println("case 22\n\nAgent " + agent.getId()
							    + " is too poor and only buys " + shout.getQuantity() + "!");
					}

					// if I can find good parameters, I can replace the previous
					// big "else" by this line:
					// shout.setQuantity( qty2buy );
					...
					if (display)
						System.out.println("case 2 source" + sourIdx + " auction="
						    + agent.auctionForWhichThisSourceBuys[sourIdx] + " agent="
						    + agent.getId() + " toReturn=" + toReturn + "TOTO");
				}
		}// for
		if (display)
		...
		for (int delIdx = 0; delIdx < agent.deliver.length; delIdx++) {
			if (agent.auctionForWhichThisDeliverSells[delIdx] == auctionIdx)
				if (agent.deliver[delIdx] > agent.deliver_capacity[delIdx]) {
					...
					shout.setPrice(valuation);
					shout
					    .setQuantity((int) (-agent.deliver_capacity[delIdx] + agent.deliver[delIdx]));
					toReturn = true;

					if (display)
						System.out.println("case 3 deliver" + delIdx + " auction="
						    + agent.auctionForWhichThisDeliverSells[delIdx] + " agent="
						    + agent.getId() + " ");
				}
			...
			if (agent.auctionForWhichThisDeliverBuys[delIdx] == auctionIdx)
				if (agent.deliver[delIdx] <= agent.deliver_capacity[delIdx]) {
					...
					if (agent.getFunds() > qty2buy * valuation) {
						shout.setQuantity(qty2buy);
						if (display)
							System.out.println("case 41 agent.getFunds()=" + agent.getFunds()
							    + " > " + qty2buy + " * valuation=" + valuation + " = "
							    + (qty2buy * valuation) + " => shout.getQuantity()="
							    + shout.getQuantity());
					} else {
					...
					if (display)
						System.out.println("case 4 deliver" + delIdx + " auction="
						    + agent.auctionForWhichThisDeliverBuys[delIdx] + " agent="
						    + agent.getId() + " ");
				}
		}// for
		...
		// System.out.println("shout.getPrice()="+shout.getPrice()+"
		// shout.getQuantity()="+shout.getQuantity());

		if ((shout.getPrice() < 0) || (shout.getQuantity() <= 0)
		    || (java.lang.Double.isNaN(shout.getPrice()))
		    || (java.lang.Double.isInfinite(shout.getPrice())))
			toReturn = false;
		 ...
		 * //shout.setPrice( shout.getPrice() + ((ManufacturerAgent)
		 * agent).getPreviousPrice( auctionIdx + 1 ) );
		 * System.out.println("SteiglitzStrategy.modifyShout: PRICE INCREASED BY
		 * "+((ManufacturerAgent) agent).getPreviousPrice( auctionIdx + 1 )+" in
		 * auction " +auctionIdx+" by "+agent.toString()+" \n"); }
		 */
		if (display) {
			System.out.print(auction.getAge()
			    + " END of SteiglitzStrategy.modifyShout: "
			    + agent.getClass().toString() + " " + agent.getId() + " (" + toReturn
			    + ") ");
			if (shout.isAsk())
				System.out.print(" SELLS ");
			if (shout.isBid())
				System.out.print(" BUYS ");
			System.out.println(shout.getQuantity() + " items in Auc" + auctionIdx
			    + " at $" + shout.getPrice() + ", previousPrice=" + previousPrice
			    + ", valuation=" + valuation + "  ***  AGENT=" + agent.toString());
		}
