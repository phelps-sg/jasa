...
public class PriceStatisticsInPeriodReport extends PriceStatisticsReport {
	...
	public void updateTransPriceLog( TransactionExecutedEvent event ) {
		...
		/*
		if( lastDisplayedTransactionPrice[ (int) ((SupplyChainRandomRobinAuction)auction).getId() ] < auction.getAge() ) {
				System.out.print("["+((SupplyChainRandomRobinAuction)auction).getId() + " "+event.getPrice()+"]");
				lastDisplayedTransactionPrice[ (int) ((SupplyChainRandomRobinAuction)auction).getId() ] = auction.getAge();
				if ( ((SupplyChainRandomRobinAuction)auction).getId() == 0 )
	...
	public void setup( ParameterDatabase parameters, Parameter base ) {
		...
		recordfrom			= parameters.getInt(base.push(P_RECORD_FROM));
		recordto			= parameters.getInt(base.push(P_RECORD_TO));
		if ( recordfrom > recordto )
			System.out.println("WARNING: "+base.push(P_RECORD_FROM)+" > "+base.push(P_RECORD_TO));
	}//setup
	
	public void produceUserOutput() {
		reportHeader();
		logger.info("WARNING: Transaction Price is given between Weeks "+recordfrom+" and "+recordto);
