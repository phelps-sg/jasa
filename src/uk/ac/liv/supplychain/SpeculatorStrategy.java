...
import ec.util.ParameterDatabase; //import edu.stanford.smi.protege.model.Facet;
...
public class SpeculatorStrategy extends FixedQuantityStrategyImpl implements
	...
	public void setup(ParameterDatabase parameters, Parameter base) {
		...
		ParameterInitializer parameterInitializer = (ParameterInitializer) parameters
		    .getInstanceForParameterEq(typeParam, null, ParameterInitializer.class);
		parameterInitializer.setup(parameters, base);
		margin = parameterInitializer.getValue();

		activationdate = parameters.getIntWithDefault(base.push(P_ACTIVATION_DATE),
		    null, 0);
	...
	public boolean modifyShout(Shout.MutableShout shout) {
		...
		/*
		 * if (display) System.out.println(auction.getAge()+" SpeculatorStrategy:
		 * agent "+agent.getId()+"previousPrice="+previousPrice+ " <
		 * valuation="+valuation+" * ( 1 - margin="+margin+")="+(valuation * ( 1 -
		 * margin))+" *** "+"previousPrice="+previousPrice+ " <
		 * valuation="+valuation+" * ( 1 + margin="+margin+")="+(valuation * ( 1 +
		 * margin)));
		 */
		...
		if (previousPrice < valuation * (1 - margin)) {
			...
			/*
			 * if (display) System.out.println(auction.getAge()+" SpeculatorStrategy:
			 * BUYS ("+toReturn+") "+shout.getQuantity()+"@$"+shout.getPrice()+"
			 * SpeculatorStrategy previousPrice="+previousPrice+ " <
			 * valuation="+valuation+" * ( 1 - margin="+margin+")="+(valuation * ( 1 -
			 * margin))+agent.toString());
			 */
			...
			/*
			 * if (display) System.out.println(auction.getAge()+" SpeculatorStrategy:
			 * SELLS("+toReturn+") "+shout.getQuantity()+"@$"+shout.getPrice()+"
			 * SpeculatorStrategy previousPrice="+previousPrice+ " <
			 * valuation="+valuation+" * ( 1 + margin="+margin+")="+(valuation * ( 1 +
			 * margin))+agent.toString());
			 */
		...
		if ((shout.getQuantity() == 0)
		    || (java.lang.Double.isNaN(shout.getPrice()))
		    || (java.lang.Double.isInfinite(shout.getPrice())))
			toReturn = false;

		if (display)
			System.out.println(auction.getAge() + " speculatorStrategy "
			    + agent.getId() + " forecast=" + valuation + " previousPrice="
			    + previousPrice);

		if (display)
			if (toReturn == true) {
				if (shout.isAsk())
					System.out.print(auction.getAge() + " Speculator Agent "
					    + agent.getId() + " SELLS ");
				if (shout.isBid())
					System.out.print(auction.getAge() + " Speculator Agent "
					    + agent.getId() + " BUYS ");
				System.out.println(shout.getQuantity() + " items at $"
				    + shout.getPrice() + agent.toString());
			}
