...
//		int auctionIdx = (int) ((SupplyChainRandomRobinAuction) auction).getId();
	...
	public void eventOccurred(AuctionEvent event) {
		if (event instanceof AuctionOpenEvent) {
			SupplyChainRandomRobinAuction auction = (SupplyChainRandomRobinAuction) event
			    .getAuction();
		...
		if (event instanceof RoundClosingEvent) {
			...
			forecastedPrice = smoothingParameter * previousPrice
			    + (1 - smoothingParameter) * forecastedPrice;
		}// RoundClosingEvent
	}// eventOccurred
