...
public class SteiglitzTraderValuer implements ValuationPolicy, Serializable {
	...
	public double getSkillF() { return skillF;}
	public double getSkillG() { return skillG;}
	...
	public double determineValue(Auction auction) {
		...
		if ( ((SupplyChainAgent)agent ).sourceWhichSellsInAuction[auctionIdx] >= 0 && !( ((SupplyChainAgent)agent ).sourceWhichBuysInAuction[auctionIdx]  >= 0 ) ) {
		...
		if ( ((SupplyChainAgent)agent ).deliverWhichSellsInAuction[auctionIdx] >= 0 && !( ((SupplyChainAgent)agent ).deliverWhichBuysInAuction[auctionIdx] >= 0) ) {
		...
		//XXX System.out.println("SteiglitzTraderValuer.determineValue r="+r+" for Agent "+((SupplyChainAgent) agent).getId() + " and source_capa="+((SupplyChainAgent) agent).source_capacity[0]);
		...
		//System.out.println(auction.getAge()+" SteiglitzTraderValuer.determineValue for Agent "+ ((AbstractTradingAgent) agent).getId()+": B="+b(fbar, gbar) + ", price="+previousPrice);
	
		/*
		if ( g < 0)
			System.out.println(auction.getAge()+" SteiglitzTraderValuer.determineValue: Agent "+((SupplyChainAgent)agent ).getId()+" has gold="+g+"<0: "+((SupplyChainAgent)agent ).toString() + ", previousPrice="+previousPrice + " r="+STEIGLITZreserve);
		
		if ( fbar>1 && b(fbar, gbar)>1 ) {
			System.out.println(auction.getAge()+" SteiglitzTraderValuer.determineValue: Agent "+((SupplyChainAgent)agent ).getId()+" has fbar="+fbar+">1 and b="+b(fbar, gbar)+">1: "+((SupplyChainAgent)agent ).toString() + ", previousPrice="+previousPrice + " r="+STEIGLITZreserve);
		}
		if ( fbar<1 && b(fbar, gbar)<1 )
			System.out.println(auction.getAge()+" SteiglitzTraderValuer.determineValue: Agent "+((SupplyChainAgent)agent ).getId()+" has fbar="+fbar+"<1 and b="+b(fbar, gbar)+"<1: "+((SupplyChainAgent)agent ).toString() + ", previousPrice="+previousPrice + " r="+STEIGLITZreserve);
*/
		...
		if (auction.getAge() ==3 && ((SupplyChainAgent) agent).getId() ==15) {
			...
			System.out.println("==> 1-fbar="+(1-fbar));
			System.out.println("g="+g);
			System.out.println("r="+r);
			System.out.println("price="+previousPrice);
			System.out.println("=> gbar="+gbar);
			double b0gbar			= b0inf - ( b0inf - b00 ) * Math.exp( - gamma * gbar );
			System.out.println("==> b0gbar="+b0gbar);
			System.out.println("===> bfg="+(Math.pow( b0gbar , 1-fbar )));
			System.out.println("\n\n\n\n\n\n\n");
		}*/
	...
	public void printAgentState() {
		System.out.println("SteiglitzTraderValuer.printAgentState Agent " + ((SupplyChainAgent)agent ).getId());
		for (int i=0 ; i < ((SupplyChainAgent)agent ).source.length ; i++) {
			System.out.println("sour." + i+":"+((SupplyChainAgent)agent ).source[i]);
		}
		for (int i=0 ; i < ((SupplyChainAgent)agent ).deliver.length ; i++) {
			System.out.println("del." + i+":"+((SupplyChainAgent)agent ).deliver[i]);
		}
	}//printAgentState
	...
	public void setup( ParameterDatabase parameters, Parameter base ) {
	    ...
	    b00  = parameters.getDoubleWithDefault(base.push(P_b00),   null, minB00B01B0inf);
	    b01  = parameters.getDoubleWithDefault(base.push(P_b01),   null, minB00B01B0inf);
	    b0inf= parameters.getDoubleWithDefault(base.push(P_b0inf), null, minB00B01B0inf);
	    ...
	    //System.out.println("SteiglitzTraderValuer.setup: skillF="+skillF + " skillG="+skillG);
	}
