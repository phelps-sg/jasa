...
public class SupplyChainRandomRobinAuction extends RandomRobinAuction {
	...
	public double getTransactionPrice(int round) {
		return transactionPrice[round];
	}

	public double getTransactionPrice() {
		return transactionPrice[getAge()];
	}

	public void setNewTransactionPrice(double newPrice) {
		this.transactionPrice[getAge() + 1] = newPrice;
	}

	public void initialiseTransactionPrice(int maximumRounds) {
		transactionPrice = new double[maximumRounds + 2];
	...
	public void setup(ParameterDatabase parameters, Parameter base) {
		...
		initialPrice = parameters.getDoubleWithDefault(base.push(P_INITIAL_PRICE),
		    null, 1);
		System.out.println("Initial price in market " + getId() + " (" + getName()
		    + ") is " + initialPrice);
	}

	public void endRound() {
		/*
		 * transactionPrice[ getAge()+1 ] =
		 * ((uk.ac.liv.supplychain.ClearingHouseAuctioneer) auctioneer).getPrice();
		 * System.out.println("new price="+transactionPrice[ getAge()+1 ]);
		 */
		super.endRound();
	}

	public LinkedList getRegisteredTraders() {
		return registeredTraders;
	}

	public LinkedList getActiveTraders() {
	...
	public void run() {
		...
		if (auctioneer == null) {
			throw new AuctionRuntimeException(
			    "No auctioneer has been assigned for auction " + name);
		}
		// begin();
		try {
			// while ( !closed ) {
	...
	public void clear(Shout ask, Shout bid, double transactionPrice) {
		...
		// System.out.println(getAge()+"SupplyChainRandomRobin.clear for
		// Auction"+getId() + ": price="+"
	...
	public void clear(Shout ask, Shout bid, double buyerCharge,
		...
		buyer.getCommodityHolding()
		    .transfer(seller.getCommodityHolding(), quantity);

		int deliverIdx = seller.deliverWhichSellsInAuction[(int) getId()];
		int sourceIdx = seller.sourceWhichSellsInAuction[(int) getId()];
		if (deliverIdx >= 0) {
			...
			int availableQuantity = (int) seller.source[sourceIdx];
	...
	public void printAgentStates() {
		Iterator iter = registeredTraders.iterator();
		// SupplyChainAgent traders = (SupplyChainAgent)activeTraders
		while (iter.hasNext()) {
			SupplyChainAgent trader = (SupplyChainAgent) iter.next();
			System.out.println("Auction " + getName() + ": " + trader.toString());
		}// for
	}// printAgentStates
