...
public class SteiglitzTraderAgent extends SupplyChainAgent {
	...
	public void produce(Auction auction) {
		...
		if (((SteiglitzTraderValuer) valuer).skillG >= price * ((SteiglitzTraderValuer) valuer).skillF) {
			...
			if (display)
				System.out.println(auction.getAge()+" Ag "+getId()+ " produces " + ((SteiglitzTraderValuer) valuer).skillG+ " GOLD because skillG="+((SteiglitzTraderValuer) valuer).skillG+"> price="+price+" * skillF="+((SteiglitzTraderValuer) valuer).skillF + " = "+(price * ((SteiglitzTraderValuer) valuer).skillF));
		}
		else {
			//produce food
			...
			if (display)
				System.out.println(auction.getAge()+" Ag "+getId()+ " produces " + ((SteiglitzTraderValuer) valuer).skillF +" FOOD because skillG="+((SteiglitzTraderValuer) valuer).skillG+"<= price="+price+" * skillF="+((SteiglitzTraderValuer) valuer).skillF + " = "+(price * ((SteiglitzTraderValuer) valuer).skillF));
		}
