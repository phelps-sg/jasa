 ...
 * This only works when numintervals>=2, else a random number from [minvalue, maxvalue] is returned.
 * 
 * @author moyaux
 *
 */
...
public class DoubleFromInterval extends ParameterInitializer {
	...
	public void setup( ParameterDatabase parameters, Parameter base ) {
		...
		numberOfIntervals = parameters.getDouble(defBase.push(P_NUM_INTERVALS), null, 0);
		...
		if (numberOfIntervals<2)
			...
			firstAllocatedValue = false;
				...
				toReturn= previousAllocatedValue + ( (maxValue - minValue) / numberOfIntervals ) ;
