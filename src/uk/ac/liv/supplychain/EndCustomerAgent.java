...
public class EndCustomerAgent extends SupplyChainAgent {
	...
	public void setup(ParameterDatabase parameters, Parameter base,
		...
		consumption = parameters.getDouble(base.push(P_CONSUMPTION), base
		    .push(P_CONSUMPTION), -1);

		produceMoney = parameters.getDouble(base.push(P_PRODUCE_MONEY), base
		    .push(P_PRODUCE_MONEY), -1);

		if (deliver.length != 0)
			System.out
			    .println("Warning: Agent "
			        + getId()
			        + " of type uk.ac.liv.supplychain.RawMaterialSupplierAgent should not deliver in any market ("
			        + source.length + " are configured).");
		if (source.length == 0)
			System.out
			    .println("Warning: Agent "
			        + getId()
			        + " of type uk.ac.liv.supplychain.RawMaterialSupplierAgent should source in exactly one market (none is configured).");
		if (source.length > 1)
			System.out
			    .println("Warning: Agent "
			        + getId()
			        + " of type uk.ac.liv.supplychain.RawMaterialSupplierAgent should not source in more than one market.");
	}// setup

	public void produce(Auction auction) {
		// XXX System.out.println("AV EndCustomerAgent.simulateProduction
		// Ag="+getId()+" "+toString());
		// SupplyChainRandomRobinAuction auction =
		...
		// XXX System.out.println("AP EndCustomerAgent.simulateProduction
		// Ag="+getId()+" "+toString());
