...
public class GPSchemeIndividual extends GPIndividual implements Serializable {
   ...
   * 
   * @param treeNum
   *          The index of the tree to mute
   * @param scheme
   *          The scheme s-expression
   * @param environment
   *          A scheme list specififying the symbol->fn mapping
   */
  ...
  /**
   * Utility method for building an ArrayList from a scheme list according to
   * some mapping between scheme symbols and GPNode classes.
   */
   ...
   * 
   * @param environment
   *          A list defining a mapping between scheme symbols and GPNode class
   *          names.
   */
  ...
  public static GPNode buildNodeFromScheme( ScmObject scheme, HashMap map ) {
    ...
    if ( scheme instanceof ScmSymbol ) {
      ...
      if ( schemeNode == null ) { throw new Error("No node defined for symbol "
          + scheme); }
      try {
