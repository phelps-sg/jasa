...
public class EquilibriumSurplusLoggerTest extends TestCase 
  ...
  public void initWithParams( int ns, int nb, int tradeEnt ) {
    ...
    for( int i=0; i<ns+nb; i++ ) {
      ...
      if ( i < ns ) {
        ...
        agent.setValuationPolicy( new DailyRandomValuer(SELLER_MIN_VALUE, SELLER_MAX_VALUE) );
      } else {
        agent.setIsSeller(false);
        agent.setValuationPolicy( new DailyRandomValuer(BUYER_MIN_VALUE, BUYER_MAX_VALUE) );
      }
  ...
  public void testWithZeroTradeEnt() {
    ...
    assertTrue( "Total equilibrium surplus should be zero with no trade entitlement",
        			totEqSurplus == 0 );
  }
  ...
  public void testWithNoAgents() {
    ...
    double totEqSurplus = eqLogger.calculateTotalEquilibriumSurplus();
    assertTrue( "Total equilibrium surplus should be zero with no agents entitlement",
        			totEqSurplus == 0 );
  }
  ...
  /**
   * Check that the following stats all agree, even when we change
   *  valuations on a daily basis.
   * 
   *  - theoretical equilibrium surplus as reported by EquilibriumSurplusLogger
   *  - the actual surplus of truthful agents in a CH
   *  - theoretical equilibrium surplus as computed by this class by
   *     tallying the theoretical surplus each day.
   * 
   * @param ns
   * @param nb
   * @param tradeEnt
  ...
  public void checkTotalEqSurplus( int ns, int nb, int tradeEnt) {
    ...
    System.out.println("\ncheckTotalEqSurplus(" + ns + ", " + nb + ", " + tradeEnt + ")");
    ...
    System.out.println("As calculated using EquilibriaStats = " + computedSurplus);
    ...
    System.out.println("Ratio of actual to theoretical = " + actualToTheoretical);
    System.out.println("Ratio of computed to theoretical = " + computedToTheoretical);
  ...
  public void eventOccurred( AuctionEvent event ) {
    if ( event instanceof EndOfDayEvent ) {
      ...
      while ( i.hasNext() ) {
        ...
        if ( agent.isSeller() ) {
          agentSurplus = (eqPrice - agent.getValuation(auction)) * agent.getInitialTradeEntitlement(); 
        } else {
          agentSurplus = (agent.getValuation(auction) - eqPrice) * agent.getInitialTradeEntitlement();
        }
        if ( agentSurplus < 0 ) {
          agentSurplus = 0;
        }
